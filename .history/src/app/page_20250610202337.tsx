...
export default function Home() {
  ...
  // If remixed, randomize theme (never repeat the same theme)
  const theme: ThemeKey = remix ? themeKeys[(remix + Math.floor(Math.random()*themeKeys.length)) % themeKeys.length] : exp.theme;
  ...
  const buttonColor = isDark ? 'border-white text-white hover:bg-white/10' : 'border-black text-black hover:bg-black/10';
  ...
  return (
    <main className={`min-h-screen relative transition-colors duration-500 ${themeBg}`}>
      {/* Dynamic greeting */}
      <div className={`fixed top-20 left-1/2 -translate-x-1/2 z-50 ${greetingBg} rounded-full px-6 py-3 shadow-lg text-lg font-semibold backdrop-blur-md max-w-[90vw] text-center md:text-lg text-sm`} style={{maxWidth:'90vw'}}>
        {greeting}
      </div>
      {/* Remix button */}
      <button
        onClick={() => setRemix((r) => r+1)}
        className={`fixed bottom-6 right-6 z-50 ${greetingBg} rounded-full px-4 py-2 shadow-lg hover:opacity-90 transition md:text-base text-sm`}
