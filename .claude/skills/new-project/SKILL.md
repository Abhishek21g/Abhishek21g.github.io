...
3. Immediately after scaffolding, interview the user briefly to fill in `agent/PROJECT_CONTEXT.md` for real — don't leave it as TBD. Ask at minimum: project name, one-sentence goal, tech stack, and the install/dev/test/lint/build commands. Write the answers directly into the file.
...
5. Tell the user what was created and what's still a placeholder (e.g., "Design / Product Taste" and "Constraints" in PROJECT_CONTEXT.md are still TBD — fill in when you know them, don't guess).

## Notes

- This only ever copies files that don't already exist (`copy_if_missing` behavior) — safe to run on a partially-set-up project without clobbering existing work.
- If `~/.agent-project-template` is missing, clone it first:
  ...
  ```
- If `agent/TOOL_REGISTRY.md` already has a "Future Tool Slots" entry pending approval from the weekly scheduled check, mention it — that's the mechanism for keeping the tool list current over time, described in `agent/AUTO_UPDATE.md`.
- Global standing preferences (who Abhishek is, how he likes to work) belong in `~/.claude/CLAUDE.md` and `~/.codex/AGENTS.md`, not here — this scaffold is project-level only. If those global files don't exist yet, mention it once, but don't block on it.
