...
function maybeNotifyPlatform(platform) {
  if (!notificationsSupported() || platformLoading || !platform) return;
...
function renderNotifyBanner(platform) {
  if (!notificationsSupported() || Notification.permission !== "default" || !platform || platform.cls === "") return "";
...
function platformLastCheckedAt() {
  if (!platformData?.latest) return null;
  for (const svc of PLATFORM_SVCS) {
    const checked = platformData.latest[svc.key]?.checked_at;
...
function platformStatusLabel() {
  ...
  const checked = platformLastCheckedAt();
  const status = platform.cls === "" ? "operational" : platform.short;
  const when = checked ? ` · checked ${fmtRelative(checked)}` : "";
  return `live · ${status}${when}`;
}

function runSnapshotLabel() {
  const generated = runData?.generated_at;
  if (!generated) return "Run snapshot · none (run export-dashboard)";
  return `Run snapshot · exported ${fmtTime(generated)}`;
}
...
function render() {
  ...
  const statusLine = `${runSnapshotLabel()} | Platform ${platformStatusLabel()}`;
