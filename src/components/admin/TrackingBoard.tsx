...
function BarList({ rows, labelWidth = 'w-40' }: { rows: { key: string; count: number }[]; labelWidth?: string }) {
    ...
    <div className="flex flex-col gap-2">
      {rows.slice(0, 10).map((row) => (
        <div key={row.key} className="flex items-center gap-3">
          ...
          <span className="w-8 shrink-0 text-right font-mono text-xs text-white/50">{row.count}</span>
        </div>
      ))}
    </div>
  )
